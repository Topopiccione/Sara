...
float CircleOfConfusion(float t) {
	return max(t * .04, ( 2.0 / res_y ) * (1.0+t));
}
...
void main(void) {
	...
	vec3 cu = cross(cw,cp);
	vec3 cv = cross(cu,cw);
	vec3 dir = normalize(uv.x*cu + (uv.y + upView)*cv + 1.3*cw) + vec3(0.0, 0.0, 0.0);
